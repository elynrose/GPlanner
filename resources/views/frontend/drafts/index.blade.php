        ...
        <div class="col-md-12">
            @can('draft_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.drafts.create') }}">
                            ...
                            <tbody>
                                @foreach($drafts as $key => $draft)
                                    <tr data-entry-id="{{ $draft->id }}">
                                        ...
                                        <td>
                                            @can('draft_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.drafts.show', $draft->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('draft_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.drafts.edit', $draft->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('draft_delete')
                                                <form action="{{ route('frontend.drafts.destroy', $draft->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('draft_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.drafts.massDestroy') }}",
