    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unique_code') ? 'is-invalid' : '' }}" type="text" name="unique_code" id="unique_code" value="{{ old('unique_code', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('due_date') ? 'is-invalid' : '' }}" type="text" name="due_date" id="due_date" value="{{ old('due_date', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('actions') ? 'is-invalid' : '' }}" name="actions_id" id="actions_id">
                    @foreach($actions as $id => $entry)
                        <option value="{{ $id }}" {{ old('actions_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
