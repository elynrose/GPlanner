    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.searches.update", [$search->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $search->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $search->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" name="url" id="url">{{ old('url', $search->url) }}</textarea>
                @if($errors->has('url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('url') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('photo_url') ? 'is-invalid' : '' }}" name="photo_url" id="photo_url">{{ old('photo_url', $search->photo_url) }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('saved') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="saved" id="saved" value="1" {{ $search->saved || old('saved', 0) === 1 ? 'checked' : '' }}>
